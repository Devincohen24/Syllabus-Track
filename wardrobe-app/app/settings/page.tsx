...
function SettingsContent() {
  ...
  useEffect(() => {
    ...
    if (upgraded === '1') setFlash('Welcome to Premium! Calendar access is now unlocked.');
    if (upgraded === '0') setFlash('Upgrade cancelled — you can upgrade any time.');
  }, [searchParams]);
  ...
  const handleUpgrade = async () => {
    ...
    else { setFlash('Failed to start checkout. Make sure Stripe is configured.'); setUpgradingLoading(false); }
  };
  ...
  const handleManageBilling = async () => {
    ...
    const data = await res.json();
    if (data.url) window.location.href = data.url;
    else { setFlash('Failed to open billing portal.'); setUpgradingLoading(false); }
  };
    ...
    <div className="space-y-8">
      ...
      <section className="bg-white rounded-2xl p-6 shadow-md space-y-4">
        <div className="flex items-center justify-between">
          ...
          {isPremium && <span className="text-xs bg-indigo-100 text-indigo-700 font-bold px-3 py-1 rounded-full">PREMIUM</span>}
        </div>

        {isPremium ? (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              You have full access to all premium features including calendar integration.
              {subEnd && ` Renews ${new Date(subEnd).toLocaleDateString()}.`}
            </p>
            <button
              onClick={handleManageBilling}
              disabled={upgradingLoading}
              className="border border-gray-300 hover:border-indigo-400 text-gray-700 font-medium px-4 py-2 rounded-xl text-sm transition-colors"
            >
              Manage Billing
            </button>
          </div>
          ...
          <div className="space-y-3">
            <div className="bg-indigo-50 rounded-xl p-4 space-y-2">
              <p className="font-semibold text-indigo-900 text-sm">StyleAI Premium — $0.99/month</p>
              <ul className="text-sm text-indigo-700 space-y-1">
                <li>✓ Calendar integration (see today&apos;s events)</li>
                <li>✓ Smarter outfit picks based on your schedule</li>
